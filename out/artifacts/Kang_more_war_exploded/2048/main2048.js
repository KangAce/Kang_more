...
function generateOneNumber() {
    ...
    //随机一个位置
    var randx = parseInt(Math.floor(Math.random() * 4)); //math.floor(x)返回小于参数x的最大整数,即对浮点数向下取整
        ...
        for (var i = 2; i >= 0; i--) {
            ...
            if (board[i][j] != 0) {
                ...
                for (var k = 3; k > i; k--) {
                    ...
                    //落脚位置的数字和本来的数字相等 && 中间没有障碍物
                    else if (board[k][j] == board[i][j] && noBlockUp(j, i, k, board)) {
                        debugger;
                        //move
                        showMoveAnimation(i, j, k, j);
                        //add
